...
namespace FreeDraw
{
    public enum PenColor { RED, BLUE, GREEN }
    ...
    public class Drawable : MonoBehaviour
    {
        ...
        public static PenColor penColor = PenColor.RED;
...
//////////////////////////////////////////////////////////////////////////////
        ...
        public void ResetCanvasButton() => pV.RPC("ResetCanvas", RpcTarget.AllViaServer);
        ...
        void ResetCanvas()
        {
            ...
            {
                RoomManager.Instance.chatRoom.syncObject.drawXPos = new List<float>();
                RoomManager.Instance.chatRoom.syncObject.drawYPos = new List<float>();
                RoomManager.Instance.chatRoom.syncObject.penColor = new List<char>();
            }
        }
        ...
        IEnumerator DrawUpdate()
        {
            ...
            while(true)
            {
                ...
                if (!SlideManager.Instance.onSlide) continue;
                ...
                if (mouse_held_down && !no_drawing_on_current_drag)
                {
                    if (!EventSystem.current.IsPointerOverGameObject())
                    {
                        ...
                        Vector2 mouse_world_position = slideCamera.ScreenToWorldPoint(Input.mousePosition);
                        pV.RPC("DrawingProcess", RpcTarget.All, true, mouse_world_position);
                    }
                }
                // Mouse is released
                else if (!mouse_held_down && prevMouseState)
                {
                    pV.RPC("DrawingProcess", RpcTarget.All, false, Vector2.zero);
                }

                prevMouseState = mouse_held_down;
            }
        }

        [PunRPC]
        void DrawingProcess(bool isMouseDown, Vector2 mouse_world_position)
        {
            if (RoomManager.Instance.chatRoom.isSync) return;
            ...
            if (isMouseDown)
            {
                ...
                if (hit != null && hit.transform != null)
                {
                    ...
                    if (DataManager.isMaster)
                    {
                        RoomManager.Instance.chatRoom.syncObject.drawXPos.Add(mouse_world_position.x);
                        RoomManager.Instance.chatRoom.syncObject.drawYPos.Add(mouse_world_position.y);
                        switch(penColor)
                        {
                            case PenColor.RED:
                                RoomManager.Instance.chatRoom.syncObject.penColor.Add('r');
                                break;
                            case PenColor.BLUE:
                                RoomManager.Instance.chatRoom.syncObject.penColor.Add('b');
                                break;
                            case PenColor.GREEN:
                                RoomManager.Instance.chatRoom.syncObject.penColor.Add('g');
                                break;
                        }
                    }
                }
            ...
            {
                previous_drag_position = mouse_world_position;
                no_drawing_on_current_drag = false;
                if (DataManager.isMaster)
                {
                    RoomManager.Instance.chatRoom.syncObject.drawXPos.Add(previous_drag_position.x);
                    RoomManager.Instance.chatRoom.syncObject.drawYPos.Add(previous_drag_position.y);
                    RoomManager.Instance.chatRoom.syncObject.penColor.Add('n');
                }
            }

            mouse_was_previously_held_down = isMouseDown;
        }

        public IEnumerator DrawSyncProcess(List<Vector2> vecList)
        {            
            for (int i = 0; i < vecList.Count; i++)
            {
                switch(RoomManager.Instance.chatRoom.syncObject.penColor[i])
                {
                    case 'r':
                        settings.SetMarkerRed();
                        break;
                    case 'b':
                        settings.SetMarkerBlue();
                        break;
                    case 'g':
                        settings.SetMarkerGreen();
                        break;
                    default:
                        previous_drag_position = vecList[i];
                        continue;
                }
                current_brush(vecList[i]);
                yield return null;
            }
        }
    }
