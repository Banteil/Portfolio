...
{
    public class SingleYourHands : MonoBehaviour
    {
        [SerializeField]
        ...
        public void Refill()
        {
            ...
            // 카드 위치 셔플
            int randomNum = Random.Range(0, Define.MAX_HANDS);
            Define.CardType tempCardType = cardList[0].Type;
            cardList[0].SetCard(cardList[randomNum].Type);
            cardList[randomNum].SetCard(tempCardType);
        ...
        public void SubmitAnyCard()
        {
            ...
            if (selectedCardNum >= 0 && cardList[selectedCardNum].gameObject.activeSelf)
            {
