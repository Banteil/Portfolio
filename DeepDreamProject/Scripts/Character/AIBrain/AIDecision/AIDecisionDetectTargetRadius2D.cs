...
public class AIDecisionDetectTargetRadius2D : AIDecision
{
    ...
    public override void Initialization()
    {
        ...
        _collider = _character.BindModel.GetComponent<Collider2D>();
        _gizmoColor.a = 0.25f;
        _init = true;
    ...
    protected virtual bool DetectTarget()
    {
        ...
        for (int i = 0; i < min; i++)
        {
            ...
            if (!CanTargetSelf)
            {
                if ((_results[i].gameObject == _brain.Owner) || (_results[i].transform.IsChildOf(_brain.Owner.transform)))
                {
                    continue;
                }
            }

            Character targetCharacter = _results[i].GetComponentInParent<Character>();
            if(targetCharacter != null)
            {
                if (_character.Faction.CheckFriendly(targetCharacter.Faction))
                {
                    continue;
                }
            }
