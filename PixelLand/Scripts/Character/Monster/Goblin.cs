        ...
        if (detectionObj.CompareTag("Character"))
        {
            ...
            if (cH.CB.transform.CompareTag("Player") || cH.CB.transform.CompareTag("NPC"))
            {
                ...
                for (int i = 0; i < monsters.Count; i++)
                {
                    ...
                    monsters[i].GetComponent<Goblin>().target = target;
                    monsters[i].GetComponent<Goblin>().state = CharacterState.TRACKING;
                    monsters[i].GetComponent<Goblin>().DisplayEnemyHp();
                    monsters[i].GetComponent<Goblin>().rB.WakeUp();
                }
