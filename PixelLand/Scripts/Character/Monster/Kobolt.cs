...
public class Kobolt : MonsterBasic
{
    public float beforeDelay = 0.2f;
    public float afterDelay = 0.8f;
        ...
        if (detectionObj.CompareTag("Character"))
        {
            ...
            if (cH.CB.transform.CompareTag("Player") || cH.CB.transform.CompareTag("NPC"))
            {
                ...
                for (int i = 0; i < monsters.Count; i++)
                {
                    ...
                    monsters[i].GetComponent<Kobolt>().target = target;
                    monsters[i].GetComponent<Kobolt>().state = CharacterState.TRACKING;
                    monsters[i].GetComponent<Kobolt>().DisplayEnemyHp();
                    monsters[i].GetComponent<Kobolt>().rB.WakeUp();
                }
