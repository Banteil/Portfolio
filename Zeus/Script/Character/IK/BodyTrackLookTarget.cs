...
namespace Zeus
{
    public class BodyTrackLookTarget : MonoBehaviour
    {
        ...
        [System.Serializable]
        public class OnLookEvent : UnityEngine.Events.UnityEvent<PlayerBodyTrack> { }
        ...
        internal void EnterLook(PlayerBodyTrack vHeadTrack)
        {
            onEnterLook.Invoke(vHeadTrack);
        }

        internal void ExitLook(PlayerBodyTrack vHeadTrack)
        {
    ...
    public static class vLookTargetHelper
    {
        ...
        static LookPoints GetLookPoints(BodyTrackLookTarget lookTarget)
        {
        ...
        /// <returns></returns>
        public static bool IsVisible(this BodyTrackLookTarget lookTarget, Vector3 from, LayerMask layerMask, bool debug = false)
        {

            if (lookTarget.HideObject) return false;

            if (lookTarget.visibleCheckType == BodyTrackLookTarget.VisibleCheckType.None)
            {
            ...
            else if (lookTarget.visibleCheckType == BodyTrackLookTarget.VisibleCheckType.SingleCast)
            {
            ...
            else if (lookTarget.visibleCheckType == BodyTrackLookTarget.VisibleCheckType.BoxCast)
            {
                if (lookTarget.useLimitToDetect && Vector3.Distance(from, lookTarget.transform.position) > lookTarget.minDistanceToDetect) return false;
        ...
        /// <param name="debug">Draw lines </param>
        /// <returns></returns>
        public static bool IsVisible(this BodyTrackLookTarget lookTarget, Vector3 from, bool debug = false)
        {
            if (lookTarget.HideObject) return false;
            LookPoints points = GetLookPoints(lookTarget);
            if (lookTarget.visibleCheckType == BodyTrackLookTarget.VisibleCheckType.None)
